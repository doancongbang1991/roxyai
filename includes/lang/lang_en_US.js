...
var unknownrequest = "Sorry I could not understand?";
//handle request
var voiceRequestHandlers_en_US = [
    ...
    new VoiceRequestHandler([
        /^transport (from)( )?([- a-z0-9]+) (to)( )?([- a-z0-9]+)$/gi
    ], function (matchGroups) {
        openInNewTab("https://www.rome2rio.com/s/" + matchGroups[3] + "/" + matchGroups[6], openintab, allowpopup);
    }),
    new VoiceRequestHandler([
    ...
    new VoiceRequestHandler([
        /^(how to cook)( )?([- a-z0-9]+)$/gi
    ], function (matchGroups) {
        console.log(matchGroups);
        cookrecipe(matchGroups[3], "Get list recipes");
        //voiceAssistant.say("Language change to" + matchGroups[3], true);
    }),
    new VoiceRequestHandler([
        /^flight (from)( )?([- a-z0-9]+) (to)( )?([- a-z0-9]+) (on)( )?([- a-z0-9]+)$/gi
    ], function (matchGroups) {
        console.log(matchGroups);
        getFlight(matchGroups[3], matchGroups[6], matchGroups[9], "Get flight information ");
    }),
    new VoiceRequestHandler([
        /^qpx (from)( )?([- a-z0-9]+) (to)( )?([- a-z0-9]+) (on)( )?([- a-z0-9]+)$/gi
    ], function (matchGroups) {
        console.log(matchGroups);
        getFlightQPX(matchGroups[3], matchGroups[6], matchGroups[9], "Get flight information ");
    }),
    new VoiceRequestHandler([
        "stop"
    ], function () {
